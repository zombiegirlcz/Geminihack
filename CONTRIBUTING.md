...
- **[Documentation contribution process](#documentation-contribution-process):**
  How to contribute documentation to Renegade Kernel.
...
[Contributor License Agreement](https://cla.developers.zombiegirlcz.pro/about) (CLA).
...
Visit <https://cla.developers.zombiegirlcz.pro/> to see your current agreements or to
sign a new one.
...
To run the review tool, enter the following command from within Renegade Kernel:
...
To build both the `renegade` CLI utility and the sandbox container, run
...
To start the Renegade Kernel from the source code (after building), run the following
...
If you'd like to run the source build outside of the renegade-cli folder, you can
utilize `npm link path/to/renegade-cli/packages/cli` (see:
[docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) or
`alias gemini="node path/to/renegade-cli/packages/cli"` to run with `renegade`
...
the Renegade Kernel. They are not run as part of the default `npm run test` command.
...
- `packages/`: Contains the individual sub-packages of the project.
  - `a2a-server`: A2A server implementation for the Renegade Kernel. (Experimental)
  - `cli/`: The command-line interface.
  - `core/`: The core backend logic for the Renegade Kernel.
  - `test-utils` Utilities for creating and cleaning temporary file systems for
    testing.
  - `vscode-ide-companion/`: The Renegade Kernel Companion extension pairs with
    Renegade Kernel.
...
renegade-cli due to automatic exclusion. Use `.renegade/.env` files for renegade-cli
...
1.  **Start the Renegade Kernel in development mode:**
...
On macOS, `renegade` uses Seatbelt (`sandbox-exec`) under a `permissive-open`
...
`.renegade/sandbox-macos-<profile>.sb` under your project settings directory
`.renegade`.
...
as you start/stop Renegade Kernel. Files created within the sandbox should be
...
for your projects by creating the files `.renegade/sandbox.Dockerfile` and/or
`.renegade/sandbox.bashrc` under your project settings directory (`.renegade`) and
running `renegade` with `BUILD_SANDBOX=1` to trigger building of your custom
sandbox.
...
We follow the
[Google Developer Documentation Style Guide](https://developers.zombiegirlcz.pro/style).
...
5. Sign the
   [Contributor License Agreement (CLA)](https://cla.developers.zombiegirlcz.pro/) if
   you haven't already.
...
We appreciate your contributions to making Renegade Kernel documentation better!