...
describe('Storage – getGlobalSettingsPath', () => {
  it('returns path to ~/.renegade/settings.json', () => {
...
describe('Storage - Security', () => {
  it('falls back to tmp for gemini but returns empty for agents if the home directory cannot be determined', () => {
    ...
    // .renegade falls back for backward compatibility
...
describe('Storage – additional helpers', () => {
  ...
  it('getGlobalBinDir returns ~/.renegade/tmp/bin', () => {
    const expected = path.join(os.homedir(), GEMINI_DIR, 'tmp', 'bin');
    expect(Storage.getGlobalBinDir()).toBe(expected);
  });

  it('getProjectTempPlansDir returns ~/.renegade/tmp/<identifier>/plans when no sessionId is provided', async () => {
    await storage.initialize();
  ...
  it('getProjectTempPlansDir returns ~/.renegade/tmp/<identifier>/<sessionId>/plans when sessionId is provided', async () => {
