...
vi.mock('../tools/memoryTool', () => ({
  ...
  getCurrentGeminiMdFilename: vi.fn(() => 'renegade.md'), // Mock the original filename
  DEFAULT_CONTEXT_FILENAME: 'renegade.md',
  GEMINI_DIR: '.renegade',
}));
...
describe('Config JIT Initialization', () => {
  ...
  beforeEach(() => {
    ...
    mockContextManager = {
      ...
      getLoadedPaths: vi.fn().mockReturnValue(new Set(['/path/to/renegade.md'])),
  ...
  it('should initialize ContextManager, load memory, and delegate to it when experimentalJitContext is enabled', async () => {
    ...
    expect(config.getGeminiMdFilePaths()).toEqual(['/path/to/renegade.md']);
  });
