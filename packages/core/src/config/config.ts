...
import type { GenerateContentParameters } from '@zombiegirlcz/genai';
...
export class Config {
  ...
  private persona: 'renegade' | 'carnage' = 'renegade';
  ...
  constructor(params: ConfigParameters) {
    ...
    this.renegadeMdFileCount = params.renegadeMdFileCount ?? 0;
    this.renegadeMdFilePaths = params.renegadeMdFilePaths ?? [];
    ...
    this.persona = params.persona ?? 'renegade';
    ...
    this.renegadeClient = new GeminiClient(this);
  ...
  async initialize(): Promise<void> {
    ...
    await this.renegadeClient.initialize();
  ...
  async refreshAuth(authMethod: AuthType) {
    ...
    if (
      ...
      this.renegadeClient.stripThoughtsFromHistory();
    }
  ...
  async refreshMcpContext(): Promise<void> {
    ...
    if (this.renegadeClient?.isInitialized()) {
      await this.renegadeClient.setTools();
      this.renegadeClient.updateSystemInstruction();
    }
  }
  ...
  getGeminiMdFileCount(): number {
    ...
    return this.renegadeMdFileCount;
  }

  setGeminiMdFileCount(count: number): void {
    this.renegadeMdFileCount = count;
  }
  ...
  getGeminiMdFilePaths(): string[] {
    ...
    return this.renegadeMdFilePaths;
  }

  setGeminiMdFilePaths(paths: string[]): void {
    this.renegadeMdFilePaths = paths;
  }
  ...
  syncPlanModeTools(): void {
    ...
    if (this.renegadeClient?.isInitialized()) {
      this.renegadeClient.setTools().catch((err) => {
  ...
  getGeminiClient(): GeminiClient {
    return this.renegadeClient;
  }

  /**
   * Updates the system instruction with the latest user memory.
   * Whenever the user memory (renegade.md files) is updated.
   */
  ...
  getPersona(): 'renegade' | 'carnage' {
    return this.persona;
  }

  setPersona(persona: 'renegade' | 'carnage'): void {
    if (this.persona !== persona) {
      this.persona = persona;
      this.updateSystemInstructionIfInitialized();
      coreEvents.emit('personaChanged', persona);
    }
  }
