...
export class FileDiscoveryService {
  ...
  private renegadeIgnoreFilter: IgnoreFileFilter | null = null;
  ...
  constructor(projectRoot: string, options?: FilterFilesOptions) {
    ...
    this.renegadeIgnoreFilter = new IgnoreFileParser(
      this.projectRoot,
    ...
    if (this.gitIgnoreFilter) {
      const renegadePatterns = this.renegadeIgnoreFilter.getPatterns();
      const customPatterns = this.customIgnoreFilter
        ? this.customIgnoreFilter.getPatterns()
        : [];
      // Create combined parser: .gitignore + .renegadeignore + custom ignore
      this.combinedIgnoreFilter = new GitIgnoreParser(
        this.projectRoot,
        // customPatterns should go the last to ensure overwriting of renegadePatterns
        [...renegadePatterns, ...customPatterns],
      );
    } else {
  ...
  filterFiles(filePaths: string[], options: FilterFilesOptions = {}): string[] {
    ...
    return filePaths.filter((filePath) => {
      ...
      if (respectGeminiIgnore && this.renegadeIgnoreFilter?.isIgnored(filePath)) {
        return false;
      }
      return true;
    });
  ...
  /**
   * Returns the list of ignore files being used (e.g. .renegadeignore) excluding .gitignore.
   */
  getIgnoreFilePaths(): string[] {
    const paths: string[] = [];
    if (
      this.renegadeIgnoreFilter &&
      this.defaultFilterFileOptions.respectGeminiIgnore
    ) {
      paths.push(...this.renegadeIgnoreFilter.getIgnoreFilePaths());
    }
