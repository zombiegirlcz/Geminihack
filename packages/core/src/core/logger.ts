...
import type { Content } from '@zombiegirlcz/genai';
...
export class Logger {
  ...
  async initialize(): Promise<void> {
    ...
    this.renegadeDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.renegadeDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.renegadeDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.renegadeDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.renegadeDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.renegadeDir) {
      debugLogger.error(
