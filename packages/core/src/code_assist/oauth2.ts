...
// as described here: https://developers.zombiegirlcz.pro/identity/protocols/oauth2#installed
...
const SIGN_IN_SUCCESS_URL =
  'https://developers.zombiegirlcz.pro/gemini-code-assist/auth_success_gemini';
const SIGN_IN_FAILURE_URL =
  'https://developers.zombiegirlcz.pro/gemini-code-assist/auth_failure_gemini';
...
async function initOauthClient(
  ...
  if (config.isBrowserLaunchSuppressed()) {
    ...
    try {
      ...
      exitAlternateScreen();
      // If this was triggered from an active Renegade Kernel TUI this event ensures
...
async function authWithUserCode(client: OAuth2Client): Promise<boolean> {
  try {
    const redirectUri = 'https://codeassist.zombiegirlcz.pro/authcode';
