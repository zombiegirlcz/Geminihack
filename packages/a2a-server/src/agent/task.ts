...
import type { PartUnion, Part as genAiPart } from '@zombiegirlcz/genai';
...
export class Task {
  ...
  private constructor(
    ...
    this.renegadeClient = this.config.getGeminiClient();
  ...
  addToolResponsesToHistory(completedTools: CompletedToolCall[]): void {
    ...
    for (const response of responsesToAdd) {
      ...
      this.renegadeClient.addHistory({
        role: 'user',
        parts,
      });
    }
  ...
  async *sendCompletedToolsToLlm(
    ...
    this.setTaskStateAndPublishUpdate('working', stateChange);
    yield* this.renegadeClient.sendMessageStream(
      llmParts,
      aborted,
