...
import type { ToolActionReturn } from '@zombiegirlcz/RENEGADEkarnelcli-core';
import { debugLogger } from '@zombiegirlcz/RENEGADEkarnelcli-core';
...
describe('setupGithubCommand', async () => {
  ...
  it('downloads workflows, updates gitignore, and includes pipefail on non-windows', async () => {
    ...
    if (gitignoreExists) {
      ...
      expect(gitignoreContent).toContain('.renegade/');
...
describe('updateGitignore', () => {
  ...
  it('does not add duplicate entries', async () => {
    const gitignorePath = path.join(scratchDir, '.gitignore');
    const existingContent = '.renegade/\nsome-other-file\ngha-creds-*.json\n';
    await fs.writeFile(gitignorePath, existingContent);
  ...
  it('adds only missing entries when some already exist', async () => {
    ...
    // Should not duplicate .renegade/ entry
    expect((content.match(/\.renegade\//g) || []).length).toBe(1);
  });

  it('does not get confused by entries in comments or as substrings', async () => {
    const gitignorePath = path.join(scratchDir, '.gitignore');
    const existingContent = [
      '# This is a comment mentioning .renegade/ folder',
      'my-app.renegade/config',
    ...
    expect(content).toContain('.renegade/');
    expect(content).toContain('gha-creds-*.json');
    ...
    const lines = content
      ...
      .filter((line) => line);
    expect(lines).toContain('.renegade/');
    expect(lines).toContain('gha-creds-*.json');
    expect(lines).toContain('my-app.renegade/config');
