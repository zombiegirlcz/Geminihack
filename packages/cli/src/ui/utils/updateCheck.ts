...
import { getPackageJson, debugLogger } from '@zombiegirlcz/RENEGADEkarnelcli-core';
  ...
  try {
    ...
    if (isNightly) {
      ...
      if (bestUpdate && semver.gt(bestUpdate, currentVersion)) {
        const message = `A new version of Renegade Kernel is available! ${currentVersion} → ${bestUpdate}`;
      ...
      if (latestUpdate && semver.gt(latestUpdate, currentVersion)) {
        const message = `Renegade Kernel update available! ${currentVersion} → ${latestUpdate}`;
