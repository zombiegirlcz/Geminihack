...
import { debugLogger, getErrorMessage } from '@zombiegirlcz/RENEGADEkarnelcli-core';
...
export async function handleMigrateFromClaude() {
  ...
  try {
    ...
    debugLogger.log('✓ Hooks successfully migrated to .renegade/settings.json');
    debugLogger.log(
      '\nMigration complete! Please review the migrated hooks in .renegade/settings.json',
    );
  } catch (error) {
...
export const migrateCommand: CommandModule = {
  command: 'migrate',
  describe: 'Migrate hooks from Claude Code to Renegade Kernel',
  builder: (yargs) =>
  ...
  handler: async (argv) => {
    ...
    if (args.fromClaude) {
      ...
      debugLogger.log(
        'Usage: renegade hooks migrate --from-claude\n\nMigrate hooks from Claude Code to Renegade Kernel format.',
      );
    }
    await exitCli();
  },
