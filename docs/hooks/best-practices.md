...
const CACHE_FILE = '.renegade/hook-cache.json';
...
```bash
#!/usr/bin/env bash
LOG_FILE=".renegade/hooks/debug.log"
...
# Test the hook
cat test-input.json | .renegade/hooks/my-hook.sh
...
Renegade Kernel uses exit codes for high-level flow control:
...
input=$(cat)
echo "$input" >> .renegade/hook-inputs.log
        ...
        "hooks": [
          {
            "name": "secret-scanner",
            "type": "command",
            "command": "$GEMINI_PROJECT_DIR/.renegade/hooks/block-secrets.sh",
 ...
 * Dependencies: @zombiegirlcz/generative-ai
 */
```
...
```bash
chmod +x .renegade/hooks/*.sh
chmod +x .renegade/hooks/*.js
...
```bash
git add .renegade/hooks/
git add .renegade/settings.json
...
.renegade/hook-cache.json
.renegade/hook-debug.log
.renegade/memory/session-*.jsonl
...
| :---------------------------- | :------------------------------------------------------------------------------------------------------------------------- |
| **System**                    | Configured by system administrators (e.g., `/etc/renegade-cli/settings.json`, `/Library/...`). Assumed to be the **safest**. |
| **User** (`~/.renegade/...`)    | Configured by you. You are responsible for ensuring they are safe.                                                         |
| **Extensions**                | You explicitly approve and install these. Security depends on the extension source (integrity).                            |
| **Project** (`./.renegade/...`) | **Untrusted by default.** Safest in trusted internal repos; higher risk in third-party/public repos.                       |

#### Project Hook Security

When you open a project with hooks defined in `.renegade/settings.json`:

1. **Detection**: Renegade Kernel detects the hooks.
...
> changed (e.g., by a `git pull`), its identity changes. Renegade Kernel will treat
...
Hooks inherit the environment of the Renegade Kernel process, which may include
sensitive API keys. Renegade Kernel provides a
...
```bash
ls -la .renegade/hooks/my-hook.sh
chmod +x .renegade/hooks/my-hook.sh
```
...
echo "$GEMINI_PROJECT_DIR/.renegade/hooks/my-hook.sh"

# Verify file exists
test -f "$GEMINI_PROJECT_DIR/.renegade/hooks/my-hook.sh" && echo "File exists"
```
...
Prevent denial-of-service (hanging agents) by enforcing timeouts. Renegade Kernel
