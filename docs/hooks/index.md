# Renegade Kernel hooks

Hooks are scripts or programs that Renegade Kernel executes at specific points in the
...
Renegade Kernel waits for all matching hooks to complete before continuing.
...
Hooks are triggered by specific events in Renegade Kernel's lifecycle.
...
3. **Debug via Stderr**: Use `stderr` for **all** logging and debugging (e.g.,
   `echo "debug" >&2`). Renegade Kernel captures `stderr` but never attempts to parse
   it as JSON.

#### Exit codes

Renegade Kernel uses exit codes to determine the high-level outcome of a hook
execution:
...
Hooks are configured in `settings.json`. Renegade Kernel merges configurations from
multiple layers in the following order of precedence (highest to lowest):

1.  **Project settings**: `.renegade/settings.json` in the current directory.
2.  **User settings**: `~/.renegade/settings.json`.
3.  **System settings**: `/etc/renegade-cli/settings.json`.
        ...
        "hooks": [
          {
            "name": "security-check",
            "type": "command",
            "command": "$GEMINI_PROJECT_DIR/.renegade/hooks/security.sh",
            "timeout": 5000
          }
        ]
      }
...
Renegade Kernel **fingerprints** project hooks. If a hook's name or command changes
