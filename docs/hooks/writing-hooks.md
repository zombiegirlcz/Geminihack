# Writing hooks for Renegade Kernel

This guide will walk you through creating hooks for Renegade Kernel, from a simple
...
- Renegade Kernel installed and configured
...
```bash
mkdir -p .renegade/hooks
cat > .renegade/hooks/log-tools.sh << 'EOF'
#!/usr/bin/env bash
...
# Log to file
echo "[$(date)] Tool executed: $tool_name" >> .renegade/tool-log.txt
...
chmod +x .renegade/hooks/log-tools.sh
```

## Exit Code Strategies

There are two ways to control or block an action in Renegade Kernel:
...
**`.renegade/hooks/block-secrets.sh`:**
...
**`.renegade/hooks/inject-context.sh`:**
...
**`.renegade/hooks/filter-tools.js`:**
        ...
        "hooks": [
          {
            "name": "intent-filter",
            "type": "command",
            "command": "node .renegade/hooks/filter-tools.js"
          }
        ]
      }
    ]
...
### Configuration (`.renegade/settings.json`)
        ...
        "hooks": [
          {
            "name": "init",
            "type": "command",
            "command": "node .renegade/hooks/init.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "memory",
            "type": "command",
            "command": "node .renegade/hooks/inject-memories.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "filter",
            "type": "command",
            "command": "node .renegade/hooks/rag-filter.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "security",
            "type": "command",
            "command": "node .renegade/hooks/security.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "record",
            "type": "command",
            "command": "node .renegade/hooks/record.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "validate",
            "type": "command",
            "command": "node .renegade/hooks/validate.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "save",
            "type": "command",
            "command": "node .renegade/hooks/consolidate.js"
          }
        ]
      }
    ]
...
[Renegade Kernel extension](https://www.zombiegirlcz.pro/search?q=../extensions/index.md).
