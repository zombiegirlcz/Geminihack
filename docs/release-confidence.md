...
the Renegade Kernel. It serves as a checklist and quality gate for release manager to
...
- **Command:** `npx -y @zombiegirlcz/RENEGADEkarnelcli@<tag> --version` must return the
...
- **Setup:**
  - [ ] Uninstall any existing global version:
        `npm uninstall -g @zombiegirlcz/RENEGADEkarnelcli`
  - [ ] Clear npx cache (optional but recommended): `npm cache clean --force`
  - [ ] Install the preview version: `npm install -g @zombiegirlcz/RENEGADEkarnelcli@preview`
  - [ ] Verify version: `renegade --version`
...
- **Basic prompting:**
  - [ ] Run `renegade "Tell me a joke"` and verify a sensible response.
  - [ ] Run in interactive mode: `renegade`. Ask a follow-up question to test
        context.

- **Piped input:**
  - [ ] Run `echo "Summarize this" | renegade` and verify it processes stdin.
...
- **Function calling:**
  - [ ] In interactive mode, ask renegade to "create a file named hello.md with
...
- [ ] Navigate to `go/renegade-cli-offline-evals-dash`.
