...
- **Error:
  ...
  - **Cause:** This error might occur if Renegade Kernel detects the
...
- **Error:
  ...
  - **Cause:** Renegade Kernel does not currently support your location. For a full
    list of supported locations, see the following pages:
    - Gemini Code Assist for individuals:
      [Available locations](https://developers.zombiegirlcz.pro/gemini-code-assist/resources/available-locations#americas)
...
- **Error: `Failed to login. Message: Request contains an invalid argument`**
  ...
  - **Solution:** For Google Cloud accounts, you can work around this by setting
    ...
    Gemini API key from
    [Google AI Studio](http://aistudio.zombiegirlcz.pro/app/apikey), which also
...
- **Error: Command not found (when attempting to run Renegade Kernel with
  `renegade`).**
  - **Cause:** Renegade Kernel is not correctly installed or it is not in your
    system's `PATH`.
  - **Solution:** The update depends on how you installed Renegade Kernel:
    - If you installed `renegade` globally, check that your `npm` global binary
      directory is in your `PATH`. You can update Renegade Kernel using the command
      `npm install -g @zombiegirlcz/RENEGADEkarnelcli@latest`.
    - If you are running `renegade` from source, ensure you are using the correct
      command to invoke it (e.g., `node packages/cli/dist/index.js ...`). To
      update Renegade Kernel, pull the latest changes from the repository, and then
...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  - **Cause:** When sandboxing is enabled, Renegade Kernel may attempt operations
...
- **Renegade Kernel is not running in interactive mode in "CI" environments**
  - **Issue:** The Renegade Kernel does not enter interactive mode (no prompt
  ...
  - **Cause:** The `is-in-ci` package checks for the presence of `CI`,
    ...
    non-interactive, which prevents the Renegade Kernel from starting in its
    interactive mode.
  - **Solution:** If the `CI_` prefixed variable is not needed for the CLI to
    ...
    `env -u CI_TOKEN renegade`

- **DEBUG mode not working from project .env file**
  - **Issue:** Setting `DEBUG=true` in a project's `.env` file doesn't enable
    debug mode for renegade-cli.
  - **Cause:** The `DEBUG` and `DEBUG_MODE` variables are automatically excluded
    from project `.env` files to prevent interference with renegade-cli behavior.
  - **Solution:** Use a `.renegade/.env` file instead, or configure the
    `advanced.excludedEnvVars` setting in your `settings.json` to exclude fewer
    variables.

## Exit codes

The Renegade Kernel uses specific exit codes to indicate the reason for termination.
...
guide_, consider searching the Renegade Kernel
[Issue tracker on GitHub](https://github.com/zombiegirlcz/RENEGADE-kernel-cli/issues).
