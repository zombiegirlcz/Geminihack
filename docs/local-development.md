...
2.  **Run Renegade Kernel with dev tracing:**

    In a separate terminal, run your Renegade Kernel command with the
    `GEMINI_DEV_TRACING` environment variable:

    ```bash
    GEMINI_DEV_TRACING=true renegade
    ```
...
```typescript
import { runInDevTraceSpan } from '@zombiegirlcz/RENEGADEkarnelcli-core';
