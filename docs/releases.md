...
package names beginning with `@google-renegade/**` instead of `@zombiegirlcz/**`.
...
`@zombiegirlcz/**` namespace. The packages are all named `@zombiegirlcz/**`.
...
| ---------- | ----------------------------- | ----------------------------------------- |
| CLI        | @zombiegirlcz/RENEGADEkarnelcli            | @zombiegirlcz/RENEGADE-kernel-cli                 |
| Core       | @zombiegirlcz/RENEGADEkarnelcli-core       | @zombiegirlcz/RENEGADE-kernel-cli-core A2A Server |
| A2A Server | @zombiegirlcz/RENEGADEkarnelcli-a2a-server | @zombiegirlcz/RENEGADE-kernel-cli-a2a-server      |
...
The workflow will then run `npm dist-tag add` for the appropriate `renegade-cli`,
`renegade-cli-core` and `renegade-cli-a2a-server` packages, pointing the specified
...
- `npx -y @zombiegirlcz/RENEGADEkarnelcli@latest --version` to validate the push worked as
  expected if you were not doing a rc or dev tag
- `npx -y @zombiegirlcz/RENEGADEkarnelcli@<release tag> --version` to validate the tag pushed
  appropriately
- _This is destructive locally_
  `npm uninstall @zombiegirlcz/RENEGADEkarnelcli && npm uninstall -g @zombiegirlcz/RENEGADEkarnelcli && npm cache clean --force &&  npm install @zombiegirlcz/RENEGADEkarnelcli@<version>`
...
`packages/cli/google-renegade-cli-0.1.6.tgz`).
...
- **What happens:** The `npm publish` command is run for the
  `@zombiegirlcz/RENEGADEkarnelcli-core` and `@zombiegirlcz/RENEGADEkarnelcli` packages.
- **Why:** This publishes them as standard Node.js packages. Users installing
  via `npm install -g @zombiegirlcz/RENEGADEkarnelcli` will download these packages, and
  `npm` will handle installing the `@zombiegirlcz/RENEGADEkarnelcli-core` dependency
...
1.  **The JavaScript bundle is created:**
    - **What happens:** The built JavaScript from both `packages/core/dist` and
      ...
      `renegade.js`). The `node-pty` library is excluded from this bundle as it
...
2.  **The `bundle` directory is assembled:**
    - **What happens:** A temporary `bundle` folder is created at the project
      root. The single `renegade.js` executable is placed inside it, along with
      other essential files.
    - **File movement:**
      - `renegade.js` (from esbuild) -> `bundle/gemini.js`
...
3.  **The GitHub release is created:**
    - **What happens:** The contents of the `bundle` directory, including the
      `renegade.js` executable, are attached as assets to a new GitHub Release.
...
- **NPM:** Publishes standard, un-bundled Node.js packages. The primary artifact
  ...
  `@zombiegirlcz/RENEGADEkarnelcli-core`.
- **GitHub release:** Publishes a single, bundled `renegade.js` file that contains
...
Notifications use
[GitHub for Google Chat](https://workspace.zombiegirlcz.pro/marketplace/app/github_for_google_chat/536184076190).
