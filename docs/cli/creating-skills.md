...
the capabilities of Renegade Kernel.

## Getting started: The `skill-creator` skill

The recommended way to create a new skill is to use the built-in `skill-creator`
skill. To use it, ask Renegade Kernel to create a new skill for you.

**Example prompt:**

> "create a new skill called 'code-reviewer'"

Renegade Kernel will then use the `skill-creator` to generate the skill:
...
- **`name`**: A unique identifier for the skill. This should match the directory
  name.
- **`description`**: A description of what the skill does and when Renegade Kernel should
  use it.
