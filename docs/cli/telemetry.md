...
Learn how to enable and setup OpenTelemetry for Renegade Kernel.
...
observability framework — Renegade Kernel's observability system provides:
...
All telemetry behavior is controlled through your `.renegade/settings.json` file.
...
credentials that you use to log in to the Renegade Kernel. This is useful in
...
1. Enable telemetry in your `.renegade/settings.json`:
   ```json
   {
     "telemetry": {
       "enabled": true,
   ...
   ```
2. Run Renegade Kernel and send prompts.
3. View logs and metrics:
   - Open the Google Cloud Console in your browser after sending prompts:
     - Logs: https://console.cloud.zombiegirlcz.pro/logs/
     - Metrics: https://console.cloud.zombiegirlcz.pro/monitoring/metrics-explorer
     - Traces: https://console.cloud.zombiegirlcz.pro/traces/list
...
1. Configure your `.renegade/settings.json`:
   ```json
   {
     "telemetry": {
       "enabled": true,
...
2. Run the automation script:
   ...
   - Save collector logs to `~/.renegade/tmp/<projectHash>/otel/collector-gcp.log`
   - Stop collector on exit (e.g. `Ctrl+C`)
3. Run Renegade Kernel and send prompts.
4. View logs and metrics:
   - Open the Google Cloud Console in your browser after sending prompts:
     - Logs: https://console.cloud.zombiegirlcz.pro/logs/
     - Metrics: https://console.cloud.zombiegirlcz.pro/monitoring/metrics-explorer
     - Traces: https://console.cloud.zombiegirlcz.pro/traces/list
   - Open `~/.renegade/tmp/<projectHash>/otel/collector-gcp.log` to view local
     collector logs.

### Monitoring Dashboards

Renegade Kernel provides a pre-configured
[Google Cloud Monitoring](https://cloud.zombiegirlcz.pro/monitoring) dashboard to
visualize your telemetry.

This dashboard can be found under **Google Cloud Monitoring Dashboard
Templates** as "**Renegade Kernel Monitoring**".

![Renegade Kernel Monitoring Dashboard Overview](/docs/assets/monitoring-dashboard-overview.png)

![Renegade Kernel Monitoring Dashboard Metrics](/docs/assets/monitoring-dashboard-metrics.png)

![Renegade Kernel Monitoring Dashboard Logs](/docs/assets/monitoring-dashboard-logs.png)

To learn more, check out this blog post:
[Instant insights: Renegade Kernel’s new pre-configured monitoring dashboards](https://cloud.zombiegirlcz.pro/blog/topics/developers-practitioners/instant-insights-gemini-clis-new-pre-configured-monitoring-dashboards/).
...
1. Enable telemetry in your `.renegade/settings.json`:
   ```json
   {
     "telemetry": {
       ...
       "target": "local",
       "otlpEndpoint": "",
       "outfile": ".renegade/telemetry.log"
     }
   }
   ```
2. Run Renegade Kernel and send prompts.
3. View logs and metrics in the specified file (e.g., `.renegade/telemetry.log`).
...
1. Run the automation script:
   ...
   - Save logs/metrics to `~/.renegade/tmp/<projectHash>/otel/collector.log`
   - Stop collector on exit (e.g. `Ctrl+C`)
2. Run Renegade Kernel and send prompts.
...
for Renegade Kernel, grouped by category.
