...
The core system instructions that guide Renegade Kernel can be completely replaced
...
via a `.renegade/.env` file. See
[Persisting Environment Variables](../get-started/authentication.md#persisting-environment-variables).

- Use the project default path (`.renegade/system.md`):
  - `GEMINI_SYSTEM_MD=true` or `GEMINI_SYSTEM_MD=1`
  - The CLI reads `./.renegade/system.md` (relative to your current project
    directory).
...
- One‑off session using a project file:
  - `GEMINI_SYSTEM_MD=1 renegade`
- Persist for a project using `.renegade/.env`:
  - Create `.renegade/system.md`, then add to `.renegade/.env`:
    - `GEMINI_SYSTEM_MD=1`
- Use a custom file under your home directory:
  - `GEMINI_SYSTEM_MD=~/prompts/SYSTEM.md renegade`
...
- Write the built‑in prompt to the project default path:
  - `GEMINI_WRITE_SYSTEM_MD=1 renegade`
- Or write to a custom path:
  - `GEMINI_WRITE_SYSTEM_MD=~/prompts/DEFAULT_SYSTEM.md renegade`

This creates the file and writes the current built‑in system prompt to it.

## Best practices: SYSTEM.md vs renegade.md
...
renegade.md focused on high‑level guidance and project specifics.

## Troubleshooting

- Error: `missing system prompt file '…'`
  - Ensure the referenced path exists and is readable.
  - For `GEMINI_SYSTEM_MD=1|true`, create `./.renegade/system.md` in your project.
- Override not taking effect
  - Confirm the variable is loaded (use `.renegade/.env` or export in your shell).
