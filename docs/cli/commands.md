...
Renegade Kernel supports several built-in commands to help you manage your session,
...
- **Description:** File an issue about Renegade Kernel. By default, the issue is
  filed within the GitHub repository for Renegade Kernel. The string you enter after
...
- **Sub-commands:**
  ...
  - **`resume <tag>`**
    ...
    - **Note:** You can only resume chats that were saved within the current
      ...
      Renegade Kernel from that project's directory.
  - **`save <tag>`**
    - **Description:** Saves the current conversation history. You must add a
      `<tag>` for identifying the conversation state.
    - **Details on checkpoint location:** The default locations for saved chat
      checkpoints are:
      - Linux/macOS: `~/.renegade/tmp/<project_hash>/`
      - Windows: `C:\Users\<YourUsername>\.renegade\tmp\<project_hash>\`
...
- **Description:** Copies the last output produced by Renegade Kernel to your
...
- **Description:** Open the Renegade Kernel documentation in your browser.
...
- **Description:** Lists all active extensions in the current Renegade Kernel
  session. See [Renegade Kernel Extensions](../extensions/index.md).

### `/help` (or `/?`)

- **Description:** Display help information about Renegade Kernel, including
  available commands and their usage.

### `/hooks`

- **Description:** Manage hooks, which allow you to intercept and customize
  Renegade Kernel behavior at specific lifecycle events.
- **Sub-commands:**
  - **`disable-all`**:
...
- **Description:** To help users easily create a `renegade.md` file, this command
  ...
  it simpler for them to provide project-specific instructions to the Renegade
  Kernel agent.
...
- **Sub-commands:**
  ...
  - **`show`**:
    - **Description:** Display the full, concatenated content of the current
      ...
      lets you inspect the instructional context being provided to the Renegade
      Kernel model.
...
- **Description:** Opens a dialog to choose your Renegade Kernel model.
...
- **Description:** Exit Renegade Kernel.
...
- **Description:** Open the settings editor to view and modify Renegade Kernel
  settings.
- **Details:** This command provides a user-friendly interface for changing
  settings that control the behavior and appearance of Renegade Kernel. It is
  equivalent to manually editing the `.renegade/settings.json` file, but with
...
- **Description:** Display detailed statistics for the current Renegade Kernel
...
- **Description:** Open a dialog that lets you change the visual theme of Renegade
  Kernel.
...
- **Features:**
  ...
  - **Persistent setting:** Vim mode preference is saved to
    `~/.renegade/settings.json` and restored between sessions
...
your prompt to Renegade Kernel. These commands include git-aware filtering.
...
- **`@<path_to_file_or_directory>`**
  ...
  - **Details:**
    ...
    - The command uses the `read_many_files` tool internally. The content is
      fetched and then inserted into your query before being sent to the Renegade
      Kernel model.
...
- **`@` (Lone at symbol)**
  - **Description:** If you type a lone `@` symbol without a path, the query is
    passed as-is to the Renegade Kernel model. This might be useful if you are
...
- If the path specified after `@` is not found or is invalid, an error message
  will be displayed, and the query might not be sent to the Renegade Kernel model, or it
...
- **`!<shell_command>`**
  - **Description:** Execute the given `<shell_command>` using `bash` on
    ...
    the terminal.
  - **Examples:**
    - `!ls -la` (executes `ls -la` and returns to Renegade Kernel)
    - `!git status` (executes `git status` and returns to Renegade Kernel)
  ...
  - **Description:** Typing `!` on its own toggles shell mode.
    - **Entering shell mode:**
      ...
      - While in shell mode, text you type is interpreted directly as a shell
        command.
    - **Exiting shell mode:**
      - When exited, the UI reverts to its standard appearance and normal Renegade
        Kernel CLI behavior resumes.
