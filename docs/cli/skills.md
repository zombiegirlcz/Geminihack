...
Agent Skills allow you to extend Renegade Kernel with specialized expertise,
...
This allows Renegade Kernel to maintain a vast library of specialized capabilities—such
as security auditing, cloud deployments, or codebase migrations—without
cluttering the model's immediate context window.

Renegade Kernel autonomously decides when to employ a skill based on your request and the
...
Renegade Kernel discovers skills from three primary locations:

1.  **Workspace Skills** (`.renegade/skills/`): Workspace-specific skills that are
    typically committed to version control and shared with the team.
2.  **User Skills** (`~/.renegade/skills/`): Personal skills available across all
    your workspaces.
...
The `renegade skills` command provides management utilities:

```bash
# List all discovered skills
renegade skills list

# Link agent skills from a local directory via symlink
# Discovers skills (SKILL.md or */SKILL.md) and creates symlinks in ~/.renegade/skills (user)
renegade skills link /path/to/my-skills-repo

# Link to the workspace scope (.renegade/skills)
renegade skills link /path/to/my-skills-repo --scope workspace

# Install a skill from a Git repository, local directory, or zipped skill file (.skill)
# Uses the user scope by default (~/.renegade/skills)
renegade skills install https://github.com/user/repo.git
renegade skills install /path/to/local/skill
renegade skills install /path/to/local/my-expertise.skill

# Install a specific skill from a monorepo or subdirectory using --path
renegade skills install https://github.com/my-org/my-skills.git --path skills/frontend-design

# Install to the workspace scope (.renegade/skills)
renegade skills install /path/to/skill --scope workspace

# Uninstall a skill by name
renegade skills uninstall my-expertise --scope workspace

# Enable a skill (globally)
renegade skills enable my-expertise

# Disable a skill. Can use --scope to specify workspace or user (defaults to workspace)
renegade skills disable my-expertise --scope workspace
```

## How it Works

1.  **Discovery**: At the start of a session, Renegade Kernel scans the discovery
    tiers and injects the name and description of all enabled skills into the
    system prompt.
2.  **Activation**: When Renegade Kernel identifies a task matching a skill's
...
Once a skill is activated (typically by Renegade Kernel identifying a task that matches
