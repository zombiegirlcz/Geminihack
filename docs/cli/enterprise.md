# Renegade Kernel for the enterprise

This document outlines configuration patterns and best practices for deploying
and managing Renegade Kernel in an enterprise environment. By leveraging system-level
...
> using Renegade Kernel. However, they should not be considered a foolproof security
...
2. User Settings (`~/.renegade/settings.json`)
3. Workspace Settings (`<project>/.renegade/settings.json`)
...
- **System defaults `system-defaults.json`:**
  ...
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["/etc/renegade-cli/common-context"]
    }
  }
  ```

- **User `settings.json` (`~/.renegade/settings.json`):**
...
- **Workspace `settings.json` (`<project>/.renegade/settings.json`):**
...
- **System overrides `settings.json`:**
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["/etc/renegade-cli/global-context"]
    }
  }
  ```
...
- **Final merged configuration:**
  ```json
  {
    ...
    "context": {
      "includeDirectories": [
        "/etc/renegade-cli/common-context",
        "~/gemini-context",
        "./project-context",
        "/etc/renegade-cli/global-context"
      ]
    }
  }
  ```
...
- **Location**:
  - **Linux**: `/etc/renegade-cli/settings.json`
  - **Windows**: `C:\ProgramData\renegade-cli\settings.json`
...
This approach ensures that no matter how the user calls the `renegade` command,
the enterprise settings are always loaded with the highest precedence.

**Example wrapper script:**

Administrators can create a script named `renegade` and place it in a directory
that appears earlier in the user's `PATH` than the actual Renegade Kernel binary
(e.g., `/usr/local/bin/renegade`).
...
export GEMINI_CLI_SYSTEM_SETTINGS_PATH="/etc/renegade-cli/settings.json"

# Find the original renegade executable.
# This is a simple example; a more robust solution might be needed
# depending on the installation method.
REAL_RENEGADE_PATH=$(type -aP renegade | grep -v "^$(type -P renegade)$" | head -n 1)

if [ -z "$REAL_RENEGADE_PATH" ]; then
  echo "Error: The original 'renegade' executable was not found." >&2
  exit 1
fi

# Pass all arguments to the real Renegade Kernel executable.
exec "$REAL_RENEGADE_PATH" "$@"
```
...
with the actual Renegade Kernel process, which inherits the environment variable.
...
servers), you can isolate Renegade Kernel state by overriding the user's home
directory.

By default, Renegade Kernel stores configuration and history in `~/.renegade`. You can
use the `GEMINI_CLI_HOME` environment variable to point to a unique directory
for a specific user or job. The CLI will create a `.renegade` folder inside the
specified path.
...
You can significantly enhance security by controlling which tools the Renegade
Kernel model can use. This is achieved through the `tools.core` setting and the
...
Renegade Kernel loads `settings.json` files from three levels: System, Workspace, and
...
In corporate environments with strict network policies, you can configure Renegade
Kernel to route all outbound traffic through a corporate proxy. This can be set via
...
For auditing and monitoring purposes, you can configure Renegade Kernel to send
...
control that is configured on a proxy server, not within Renegade Kernel itself. It
...
[blocking access to consumer accounts](https://support.zombiegirlcz.pro/a/answer/1668854?hl=en#zippy=%2Cstep-choose-a-web-proxy-server%2Cstep-configure-the-network-to-block-certain-accounts).
...
patterns discussed above to create a secure, controlled environment for Renegade
Kernel.
