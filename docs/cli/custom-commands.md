...
prompts as personal shortcuts within Renegade Kernel. You can create commands that
...
Renegade Kernel discovers commands from two locations, loaded in a specific order:

1.  **User commands (global):** Located in `~/.renegade/commands/`. These commands
    are available in any project you are working on.
2.  **Project commands (local):** Located in
    `<your-project-root>/.renegade/commands/`. These commands are specific to the
...
- A file at `~/.renegade/commands/test.toml` becomes the command `/test`.
- A file at `<project>/.renegade/commands/git/commit.toml` becomes the namespaced
  command `/git:commit`.
...
- `prompt` (String): The prompt that will be sent to the Renegade Kernel model when the
...
```toml
# In: <project>/.renegade/commands/changelog.toml
...
When a custom command attempts to execute a shell command, Renegade Kernel will now
...
````toml
# In: <project>/.renegade/commands/git/commit.toml
...
- **Directory listing**: `@{path/to/dir}` is traversed and each file present
  ...
  respects `.gitignore` and `.renegadeignore` if enabled.
...
```toml
# In: <project>/.renegade/commands/review.toml
...
```bash
mkdir -p ~/.renegade/commands/refactor
touch ~/.renegade/commands/refactor/pure.toml
```

**2. Add the content to the file:**

Open `~/.renegade/commands/refactor/pure.toml` in your editor and add the
following content. We are including the optional `description` for best
practice.

```toml
# In: ~/.renegade/commands/refactor/pure.toml
...
Renegade Kernel will then execute the multi-line prompt defined in your TOML file.