...
Renegade Kernel supports a variety of themes to customize its color scheme and
appearance. You can change the theme to suit your preferences via the `/theme`
command or `"theme":` configuration setting.

## Available themes

Renegade Kernel comes with a selection of pre-defined themes, which you can list
using the `/theme` command within Renegade Kernel:
...
1.  Enter `/theme` into Renegade Kernel.
...
Selected themes are saved in Renegade Kernel's
...
Renegade Kernel lets you create your own custom color themes by specifying them in
...
**Security note:** For your safety, Renegade Kernel will only load theme files that
...
- Select your custom theme using the `/theme` command in Renegade Kernel. Your custom
