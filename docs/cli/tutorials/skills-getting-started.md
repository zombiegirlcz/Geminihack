...
Agent Skills extend Renegade Kernel with specialized expertise. In this guide, you'll
...
1.  Run the following command to create the folders:
    ...
    ```bash
    mkdir -p .renegade/skills/api-auditor/scripts
    ```

### Create the definition

1.  Create a file at `.renegade/skills/api-auditor/SKILL.md`. This tells the agent
...
1.  Create a file at `.renegade/skills/api-auditor/scripts/audit.js`. This is the
    code the agent will run.

    ```javascript
    // .renegade/skills/api-auditor/scripts/audit.js
...
Renegade Kernel automatically discovers skills in the `.renegade/skills` directory.
...
Renegade Kernel recognizes the request matches the `api-auditor` description and asks for
...
Renegade Kernel then uses the `run_shell_command` tool to execute your bundled Node
script:

`node .renegade/skills/api-auditor/scripts/audit.js http://geminili.com`
