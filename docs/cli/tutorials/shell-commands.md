...
You can automate tasks using a combination of Renegade Kernel and shell commands.
...
1.  Renegade Kernel calls `run_shell_command('npm test')`.
2.  You see a confirmation prompt: `Allow command 'npm test'? [y/N]`.
3.  You press `y`.
4.  The tests run. If they fail, Renegade Kernel reads the error output.
5.  Renegade Kernel uses `read_file` to inspect the failing test.
6.  Renegade Kernel uses `replace` to fix the bug.
7.  Renegade Kernel runs `npm test` again to verify the fix.

This loop turns Renegade Kernel into an autonomous engineer.

## How to manage background processes

You can ask Renegade Kernel to start long-running tasks, like development servers or file
watchers.

**Prompt:** `Start the React dev server in the background.`

Renegade Kernel will run the command (e.g., `npm run dev`) and detach it.
...
Renegade Kernel attempts to handle interactive commands (like `git add -p` or
...
Giving an AI access to your shell is powerful but risky. Renegade Kernel includes
