# File management with Renegade Kernel

Explore, analyze, and modify your codebase using Renegade Kernel. In this guide,
you'll learn how to provide Renegade Kernel with files and directories, modify and
create files, and control what Renegade Kernel can see.
...
Renegade Kernel will generally try to read relevant files, sometimes prompting you
for access (depending on your settings). To ensure that Renegade Kernel uses a file,
...
If you _don't_ know the exact file path, you can ask Renegade Kernel to find it for
...
Renegade Kernel uses the `glob` or `list_directory` tools to search your project
...
Once Renegade Kernel has context, you can direct it to make specific edits. The agent
...
Renegade Kernel uses the `replace` tool to propose a targeted code change.
...
Renegade Kernel uses the `write_file` tool to generate the new file from scratch.

## Review and confirm changes

Renegade Kernel prioritizes safety. Before any file is modified, it presents a
...
Renegade Kernel uses the `run_shell_command` tool to execute your test runner (e.g.,
`npm test` or `jest`). This ensures the changes didn't break existing
functionality.

## Advanced: Controlling what Renegade Kernel sees

By default, Renegade Kernel respects your `.gitignore` file. It won't read or search
...
`.renegadeignore` file in your project root.

**Example `.renegadeignore`:**
