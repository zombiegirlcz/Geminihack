...
"uninstall" the CLI, you must clear this cache, which will remove renegade-cli and
...
If you installed the CLI globally (e.g., `npm install -g @zombiegirlcz/RENEGADEkarnelcli`),
...
`brew install renegade-cli`), use the `brew uninstall` command to remove it.

```bash
brew uninstall renegade-cli
```

## Method 4: MacPorts

If you installed the CLI globally using MacPorts (e.g.,
`sudo port install renegade-cli`), use the `port uninstall` command to remove it.

```bash
sudo port uninstall renegade-cli
```