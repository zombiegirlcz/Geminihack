# Sandboxing in the Renegade Kernel

This document provides a guide to sandboxing in the Renegade Kernel, including
prerequisites, quickstart, and configuration.

## Prerequisites

Before using sandboxing, you need to install and set up the Renegade Kernel:
...
renegade -s -p "analyze the code structure"

# Use environment variable
export GEMINI_SANDBOX=true
renegade -p "run the test suite"
...
```bash
DEBUG=1 renegade -s -p "debug command"
```
...
```bash
# Check environment
renegade -s -p "run shell command: env | grep SANDBOX"

# List mounts
renegade -s -p "run shell command: mount | grep workspace"
```
