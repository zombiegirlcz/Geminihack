...
- **Location:** Sessions are stored in `~/.renegade/tmp/<project_hash>/chats/`,
...
When starting Renegade Kernel, use the `--resume` (or `-r`) flag to load existing
sessions.
...
- **Resume by ID:** You can also provide the full session UUID:
  ```bash
  renegade --resume a1b2c3d4-e5f6-7890-abcd-ef1234567890
  ```
...
You can configure how Renegade Kernel manages your session history in your
