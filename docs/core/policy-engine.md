...
The Renegade Kernel includes a powerful policy engine that provides fine-grained
...
1.  **Create the policy directory** if it doesn't exist:
    ```bash
    mkdir -p ~/.renegade/policies
    ```
2.  **Create a new policy file** (e.g., `~/.renegade/policies/my-rules.toml`). You
    ...
    priority = 100
    ```
3.  **Run a command** that triggers the policy (e.g., ask Renegade Kernel to
...
| :------ | :--- | :------------------------------------------------------------------------- |
| Default | 1    | Built-in policies that ship with the Renegade Kernel.                           |
...
| Tier      | Type   | Location                    |
| :-------- | :----- | :-------------------------- |
| **User**  | Custom | `~/.renegade/policies/*.toml` |
...
| :---------- | :------------------------------------------------ |
| **Linux**   | `/etc/renegade-cli/policies`                        |
| **macOS**   | `/Library/Application Support/GeminiCli/policies` |
| **Windows** | `C:\ProgramData\renegade-cli\policies`              |
...
The Renegade Kernel ships with a set of default policies to provide a safe
