...
Sub-agents are specialized agents that operate within your main Renegade Kernel
...
Sub-agents are "specialists" that the main Renegade Kernel agent can hire for a specific
job.
...
Renegade Kernel comes with the following built-in sub-agents:
...
- **Name:** `cli_help`
- **Purpose:** Get expert knowledge about Renegade Kernel itself, its commands,
...
1.  **Project-level:** `.renegade/agents/*.md` (Shared with your team)
2.  **User-level:** `~/.renegade/agents/*.md` (Personal agents)
...
**Example: `.renegade/agents/security-auditor.md`**
...
Renegade Kernel can also delegate tasks to remote sub-agents using the Agent-to-Agent
(A2A) protocol.
