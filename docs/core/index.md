# Renegade Kernel core

Renegade Kernel's core package (`packages/core`) is the backend portion of Renegade
Kernel, handling communication with the Gemini API, managing tools, and processing
requests sent from `packages/cli`. For a general overview of Renegade Kernel, see the
...
While the `packages/cli` portion of Renegade Kernel provides the user interface,
...
- **Prompt engineering:** Constructing effective prompts for the Renegade Kernel model,
...
- **Tool management & orchestration:**
  - Registering available tools (e.g., file system tools, shell command
    execution).
  - Interpreting tool use requests from the Renegade Kernel model.
  - Executing the requested tools with the provided arguments.
  - Returning tool execution results to the Renegade Kernel model for further processing.
...
Renegade Kernel includes a model fallback mechanism to ensure that you can continue
...
When Renegade Kernel finds it is reciting text from a source it appends the citation to
