# Renegade Kernel core: Tools API

The Renegade Kernel core (`packages/core`) features a robust system for defining,
registering, and executing tools. These tools extend the capabilities of the
Renegade Kernel model, allowing it to interact with the local environment, fetch web
...
- **Tool (`tools.ts`):** An interface and base class (`BaseTool`) that defines
  ...
  - `description`: A clear explanation of what the tool does, which is provided
    to the Renegade Kernel model.
  - `parameterSchema`: A JSON schema defining the parameters that the tool
    accepts. This is crucial for the Renegade Kernel model to understand how to call the
    tool correctly.
...
- **Tool registry (`tool-registry.ts`):** A class (`ToolRegistry`) responsible
  ...
  - **Providing schemas:** Exposing the `FunctionDeclaration` schemas of all
    registered tools to the Renegade Kernel model, so it knows what tools are available
    and how to use them.
...
1.  **Model request:** The Renegade Kernel model, based on the user's prompt and the
...
8.  **Response to model:** The `llmContent` from the `ToolResult` is packaged as
    a `FunctionResponse` and sent back to the Renegade Kernel model so it can continue
...
- **Command-based discovery:** Advanced users or project administrators can
  ...
  by the Renegade Kernel core, should output a JSON array of `FunctionDeclaration`
...
- **MCP server(s):** For more complex scenarios, one or more MCP servers can be
  ...
  Renegade Kernel core can then discover and use tools exposed by these servers. As
...
model's capabilities, making the Renegade Kernel a versatile assistant for a wide
range of tasks.