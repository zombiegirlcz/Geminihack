...
Renegade Kernel ecosystem. Understanding these terms will help you make the most of
...
environment. Renegade Kernel provides context through several mechanisms:
...
An **Extension** is a pluggable package that adds new capabilities to Renegade
Kernel. Extensions can bundle:
...
**Headless mode** refers to running Renegade Kernel without the interactive terminal
UI (TUI). This is used for scripting, automation, and piping data into or out of
the agent.

- **Interactive:** `renegade` (starts the REPL).
- **Headless:** `renegade "Fix this file"` (runs once and exits).
...
The **Model Context Protocol (MCP)** is an open standard that allows Renegade Kernel
...
- **Use case:** Connecting Renegade Kernel to a PostgreSQL database, a GitHub repository,
