...
This guide covers the `renegade extensions` commands and the structure of the
`gemini-extension.json` configuration file.

## Extension management

We offer a suite of extension management tools using `renegade extensions`
commands.

Note that these commands (e.g. `renegade extensions install`) are not supported
...
You can install an extension using `renegade extensions install` with either a
GitHub URL or a local path.

Note that we create a copy of the installed extension, so you will need to run
`renegade extensions update` to pull in changes from both locally-defined
...
```
renegade extensions install <source> [--ref <ref>] [--auto-update] [--pre-release] [--consent]
```
...
`renegade extensions uninstall <name...>`:

```
renegade extensions uninstall renegade-cli-security renegade-cli-another-extension
```
...
```
renegade extensions disable <name> [--scope <scope>]
```
...
You can enable extensions using `renegade extensions enable <name>`. You can also
enable an extension for a specific workspace using
`renegade extensions enable <name> --scope=workspace` from within that workspace.

```
renegade extensions enable <name> [--scope <scope>]
```
...
`gemini-extension.json` `version` field) with `renegade extensions update <name>`.

You can update all extensions with:

```
renegade extensions update --all
```
...
```
renegade extensions new <path> [template]
```
...
The `renegade extensions link` command will create a symbolic link from the
extension installation directory to the development path.

This is useful so you don't have to run `renegade extensions update` every time
you make changes you'd like to test.

```
renegade extensions link <path>
```
...
On startup, Renegade Kernel looks for extensions in `<home>/.renegade/extensions`

Extensions exist as a directory that contains a `gemini-extension.json` file.
For example:

`<home>/.renegade/extensions/my-extension/gemini-extension.json`
...
When Renegade Kernel starts, it loads all the extensions and merges their
...
(e.g., `<home>/.renegade/extensions/my-api-extension/.env`).

You can view a list of an extension's settings by running:

```
renegade extensions list
```

and you can update a given setting using:

```
renegade extensions config <extension name> [setting name] [--scope <scope>]
```
...
Renegade Kernel behavior at specific lifecycle events. Hooks provided by an extension
...
```
.renegade/extensions/my-extension/
├── gemini-extension.json
└── skills/
    └── security-audit/
        └── SKILL.md
...
Renegade Kernel will automatically discover and load these agents when the extension
...
Renegade Kernel extensions allow variable substitution in both
...
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `${extensionPath}`         | The fully-qualified path of the extension in the user's filesystem e.g., '/Users/username/.renegade/extensions/example-extension'. This will not unwrap symlinks. |
