...
can install your extension using `renegade extensions install <your-repo-uri>`.
...
way `renegade extensions install <your-repo-uri> --ref=stable`. Or, you could make
...
`renegade extensions install <your-repo-uri>` is to be on the stable release
branch.
...
Renegade Kernel extensions can be distributed through
...
When checking for updates, renegade will just look for the "latest" release on
...
To ensure Renegade Kernel can automatically find the correct release asset for each
...
Renegade Kernel extension for multiple platforms:
