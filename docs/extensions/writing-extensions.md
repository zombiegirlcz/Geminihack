# Getting started with Renegade Kernel extensions

This guide will walk you through creating your first Renegade Kernel extension.
...
Before you start, make sure you have the Renegade Kernel installed and a basic
understanding of Node.js.

## When to use what

Extensions offer a variety of ways to customize Renegade Kernel.
...
```bash
renegade extensions new my-first-extension mcp-server
```
...
This is the manifest file for your extension. It tells Renegade Kernel how to load
...
- `mcpServers`: This section defines one or more Model Context Protocol (MCP)
  ...
  - `command`, `args`, `cwd`: These fields specify how to start your server.
    Notice the use of the `${extensionPath}` variable, which Renegade Kernel replaces
...
Before you can use the extension, you need to link it to your Renegade Kernel
...
2.  **Link the extension:**
    ...
    The `link` command creates a symbolic link from the Renegade Kernel extensions
    ...
    ```bash
    renegade extensions link .
    ```

Now, restart your Renegade Kernel session. The new `fetch_posts` tool will be
...
After saving the file, restart the Renegade Kernel. You can now run
...
You've successfully created a Renegade Kernel extension! You learned how to:
...
capabilities into the Renegade Kernel.