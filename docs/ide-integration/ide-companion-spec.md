# Renegade Kernel companion plugin: Interface specification

> Last Updated: September 15, 2025

This document defines the contract for building a companion plugin to enable
Renegade Kernel's IDE mode. For VS Code, these features (native diffing, context
awareness) are provided by the official extension
([marketplace](https://marketplace.visualstudio.com/items?itemName=Google.renegade-cli-vscode-ide-companion)).
...
Renegade Kernel and the IDE plugin communicate through a local communication channel.
...
For Renegade Kernel to connect, it needs to discover which IDE instance it's running
...
- **File location:** The file must be created in a specific directory:
  `os.tmpdir()/renegade/ide/`. Your plugin must create this directory if it
  doesn't exist.
