...
Renegade Kernel can integrate with your IDE to provide a more seamless and
...
- **Native diffing:** When Renegade Kernel suggests code modifications, you can view the
...
- **VS Code commands:** You can access Renegade Kernel features directly from the VS
  Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `Renegade Kernel: Run`: Starts a new Renegade Kernel session in the integrated
    terminal.
  - `Renegade Kernel: Accept Diff`: Accepts the changes in the active diff editor.
  - `Renegade Kernel: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `Renegade Kernel: View Third-Party Notices`: Displays the third-party notices for
    the extension.
...
When you run Renegade Kernel inside a supported editor, it will automatically detect
...
manually, you can run the following command inside Renegade Kernel:
...
- **For Visual Studio Code:** Install from the
  [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=google.renegade-cli-vscode-ide-companion).
...
> NOTE: The "Renegade Kernel Companion" extension may appear towards the bottom of
...
When enabled, Renegade Kernel will automatically attempt to connect to the IDE
companion extension.
...
When you ask Renegade Kernel to modify a file, it can open a diff view directly in your
editor.
...
- Open the Command Palette and run **Renegade Kernel: Accept Diff**.
...
- Open the Command Palette and run **Renegade Kernel: Close Diff Editor**.
...
If you are using Renegade Kernel within a sandbox, please be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker container:** If you run Renegade Kernel inside a Docker (or Podman)
...
- **Message:**
  ...
  - **Cause:** Renegade Kernel could not find the necessary environment variables
  ...
  - **Solution:**
    1.  Make sure you have installed the **Renegade Kernel Companion** extension in
...
- **Message:**
  `🔴 Disconnected: Directory mismatch. Renegade Kernel is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  `IDE integration is not supported in your current environment. To use this feature, run Renegade Kernel in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running Renegade Kernel in a terminal or environment that is
    not a supported IDE.
  - **Solution:** Run Renegade Kernel from the integrated terminal of a supported
    IDE, like Antigravity or VS Code.

- **Message:**
  `No installer is available for IDE. Please install the Renegade Kernel Companion extension manually from the marketplace.`
  - **Cause:** You ran `/ide install`, but the CLI does not have an automated
    installer for your specific IDE.
  - **Solution:** Open your IDE's extension marketplace, search for "Renegade Kernel
    Companion", and
