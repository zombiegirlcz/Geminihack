# Renegade Kernel planning tools

Planning tools allow the Renegade Kernel model to switch into a safe, read-only "Plan
...
- **Parameters:**
  - `plan_path` (string, required): The path to the finalized Markdown plan
    ...
    directory (e.g., `~/.renegade/tmp/<project>/plans/`).
- **Behavior:**
