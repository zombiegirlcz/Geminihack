...
The `run_shell_command` tool allows the Renegade Kernel model to execute commands
...
`settings.json` file or by using the `/settings` command in the Renegade Kernel.
...
When an interactive command is running, you can send input to it from the Renegade Kernel
...
tools to detect if they are being run from within the Renegade Kernel.
