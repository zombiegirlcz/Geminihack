# MCP servers with the Renegade Kernel
...
systems and data sources. MCP servers act as a bridge between the Renegade Kernel model
and your local environment or other services like APIs.

An MCP server enables the Renegade Kernel to:
...
With an MCP server, you can extend the Renegade Kernel's capabilities to perform
actions beyond its built-in features, such as interacting with databases, APIs,
custom scripts, or specialized workflows.

## Core integration architecture

The Renegade Kernel integrates with MCP servers through a sophisticated discovery and
...
The Renegade Kernel supports three MCP transport types:
...
prompts. Renegade Kernel discovers these automatically and gives you the possibility
...
The Renegade Kernel uses the `mcpServers` configuration in your `settings.json` file
...
The Renegade Kernel supports OAuth 2.0 authentication for remote MCP servers using
...
- **Stored securely** in `~/.renegade/mcp-oauth-tokens.json`
...
1. **[Create](https://cloud.zombiegirlcz.pro/iap/docs/oauth-client-creation) or use an
   existing OAuth 2.0 client ID.** To use an existing OAuth 2.0 client ID,
   follow the steps in
   [How to share OAuth Clients](https://cloud.zombiegirlcz.pro/iap/docs/sharing-oauth-clients).
2. **Add the OAuth ID to the allowlist for
   [programmatic access](https://cloud.zombiegirlcz.pro/iap/docs/sharing-oauth-clients#programmatic_access)
...
3. **Create a service account.**
   [Documentation](https://cloud.zombiegirlcz.pro/iam/docs/service-accounts-create#creating),
   [Cloud Console Link](https://console.cloud.zombiegirlcz.pro/iam-admin/serviceaccounts)
...
5. **Grant all users and groups** who will access the MCP Server the necessary
   permissions to
   [impersonate the service account](https://cloud.zombiegirlcz.pro/docs/authentication/use-service-account-impersonation)
   (i.e., `roles/iam.serviceAccountTokenCreator`).
6. **[Enable](https://console.cloud.zombiegirlcz.pro/apis/library/iamcredentials.googleapis.com)
...
When the Renegade Kernel starts, it performs MCP server discovery through the
...
When the Renegade Kernel model decides to use an MCP tool, the following execution flow
occurs:
...
Once discovered, MCP tools are available to the Renegade Kernel model like built-in
...
Renegade Kernel's capabilities while maintaining security, reliability, and ease of
use.
...
The Renegade Kernel will correctly process this array, separating text from binary
...
When the Renegade Kernel receives this response, it will:
...
context to the Renegade Kernel model.

## MCP prompts as slash commands

In addition to tools, MCP servers can expose predefined prompts that can be
executed as slash commands within the Renegade Kernel. This allows you to create
...
```bash
/poem-writer --title="Renegade Kernel" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Renegade Kernel" reverent
```

When you run this command, the Renegade Kernel executes the `prompts/get` method on
...
## Managing MCP servers with `renegade mcp`

While you can always configure MCP servers by manually editing your
`settings.json` file, the Renegade Kernel provides a convenient set of commands to
...
### Adding a server (`renegade mcp add`)
...
`~/.renegade/settings.json` or the project config `.renegade/settings.json` file.

**Command:**

```bash
renegade mcp add [options] <name> <commandOrUrl> [args...]
```
...
```bash
# Basic syntax
renegade mcp add [options] <name> <command> [args...]

# Example: Adding a local server
renegade mcp add -e API_KEY=123 -e DEBUG=true my-stdio-server /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
renegade mcp add python-server python server.py -- --server-arg my-value
```
...
```bash
# Basic syntax
renegade mcp add --transport http <name> <url>

# Example: Adding an HTTP server
renegade mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
renegade mcp add --transport http --header "Authorization: Bearer abc123" secure-http https://api.example.com/mcp/
```
...
```bash
# Basic syntax
renegade mcp add --transport sse <name> <url>

# Example: Adding an SSE server
renegade mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
renegade mcp add --transport sse --header "Authorization: Bearer abc123" secure-sse https://api.example.com/sse/
```

### Listing servers (`renegade mcp list`)
...
### Removing a server (`renegade mcp remove`)
...
### Enabling/disabling a server (`renegade mcp enable`, `renegade mcp disable`)
...
```bash
renegade mcp enable <name> [--session]
renegade mcp disable <name> [--session]
```
...
`~/.renegade/mcp-server-enablement.json`.
