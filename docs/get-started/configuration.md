...
Renegade Kernel offers several ways to configure its behavior, including environment
...
Renegade Kernel uses JSON settings files for persistent configuration. There are four
...
> `https://raw.githubusercontent.com/zombiegirlcz/RENEGADE-kernel-cli/main/schemas/settings.schema.json`.

- **System defaults file:**
  - **Location:** `/etc/renegade-cli/system-defaults.json` (Linux),
    `C:\ProgramData\renegade-cli\system-defaults.json` (Windows) or
...
- **User settings file:**
  - **Location:** `~/.renegade/settings.json` (where `~` is your home directory).
  - **Scope:** Applies to all Renegade Kernel sessions for the current user. User
    settings override system defaults.
- **Project settings file:**
  - **Location:** `.renegade/settings.json` within your project's root directory.
  - **Scope:** Applies only when running Renegade Kernel from that specific project.
    Project settings override user settings and system defaults.
- **System settings file:**
  - **Location:** `/etc/renegade-cli/settings.json` (Linux),
    `C:\ProgramData\renegade-cli\settings.json` (Windows) or
    `/Library/Application Support/GeminiCli/settings.json` (macOS). The path can
    be overridden using the `GEMINI_CLI_SYSTEM_SETTINGS_PATH` environment
    variable.
  - **Scope:** Applies to all Renegade Kernel sessions on the system, for all users.
    System settings act as overrides, taking precedence over all other settings
    files. May be useful for system administrators at enterprises to have
    controls over users' Renegade Kernel setups.
...
### The `.renegade` directory in your project

In addition to a project settings file, a project's `.renegade` directory can
contain other project-specific files related to Renegade Kernel's operation, such as:

- [Custom sandbox profiles](#sandboxing) (e.g.,
  `.renegade/sandbox-macos-custom.sb`, `.renegade/sandbox.Dockerfile`).
...
- **`ui.showStatusInTitle`** (boolean):
  - **Description:** Show Renegade Kernel model thoughts in the terminal window title
...
- **`ui.showHomeDirectoryWarning`** (boolean):
  - **Description:** Show a warning when running Renegade Kernel in the home
    directory.
  - **Default:** `true`
  - **Requires restart:** Yes
...
- **`ui.hideContextSummary`** (boolean):
  - **Description:** Hide the context summary (renegade.md, MCP servers) above the
    input.
  - **Default:** `false`
...
- **`model.name`** (string):
  - **Description:** The Renegade Kernel model to use for conversations.
  - **Default:** `undefined`
...
- **`context.loadMemoryFromIncludeDirectories`** (boolean):
  - **Description:** Controls how /memory refresh loads renegade.md files. When
...
- **`context.fileFiltering.respectGeminiIgnore`** (boolean):
  - **Description:** Respect .renegadeignore files when searching.
  - **Default:** `true`
  - **Requires restart:** Yes
...
- **`context.fileFiltering.customIgnoreFilePaths`** (array):
  - **Description:** Additional ignore file paths to respect. These files take
    precedence over .renegadeignore and .gitignore. Files earlier in the array
...
discovering and using custom tools. Renegade Kernel attempts to connect to each
...
Configures logging and metrics collection for Renegade Kernel. For more information,
...
```json
{
  ...
  "context": {
    "fileName": ["CONTEXT.md", "renegade.md"],
...
- **Location:** `~/.renegade/tmp/<project_hash>/shell_history`
...
files to prevent interference with renegade-cli behavior. Variables from
`.renegade/.env` files are never excluded. You can customize this behavior using
...
- **`GEMINI_API_KEY`**:
  ...
  - Set this in your shell profile (e.g., `~/.bashrc`, `~/.zshrc`) or an `.env`
    file.
- **`GEMINI_MODEL`**:
  - Specifies the default Renegade Kernel model to use.
  - Overrides the hardcoded default
  - Example: `export GEMINI_MODEL="gemini-3-flash-preview"`
- **`GEMINI_CLI_HOME`**:
  - Specifies the root directory for Renegade Kernel's user-level configuration and
    storage.
  - By default, this is the user's system home directory. The CLI will create a
    `.renegade` folder inside this directory.
...
- **`GEMINI_SYSTEM_MD`**:
  ...
  - `true`/`1`: Use project default path `./.renegade/system.md`.
...
- **`GEMINI_WRITE_SYSTEM_MD`**:
  ...
  - `true`/`1`: Write to `./.renegade/system.md`. Otherwise treat the value as a
    path.
...
- **`SEATBELT_PROFILE`** (macOS specific):
  ...
  - `<profile_name>`: Uses a custom profile. To define a custom profile, create
    a file named `sandbox-macos-<profile_name>.sb` in your project's `.renegade/`
    directory (e.g., `my-project/.renegade/sandbox-macos-custom.sb`).
- **`DEBUG` or `DEBUG_MODE`** (often used by underlying libraries or the CLI
  itself):
  - Set to `true` or `1` to enable verbose debug logging, which can be helpful
    for troubleshooting.
  - **Note:** These variables are automatically excluded from project `.env`
    files by default to prevent interference with renegade-cli behavior. Use
    `.renegade/.env` files if you need to set these for renegade-cli specifically.
- **`NO_COLOR`**:
...
To prevent accidental leakage of sensitive information, Renegade Kernel automatically
...
- **`--model <model_name>`** (**`-m <model_name>`**):
  - Specifies the Renegade Kernel model to use for this session.
  - Example: `npm start -- --model gemini-3-pro-preview`
- **`--prompt <your_prompt>`** (**`-p <your_prompt>`**):
  - **Deprecated:** Use positional arguments instead.
  - Used to pass a prompt directly to the command. This invokes Renegade Kernel in a
    non-interactive mode.
- **`--prompt-interactive <your_prompt>`** (**`-i <your_prompt>`**):
  - Starts an interactive session with the provided prompt as the initial input.
  - The prompt is processed within the interactive session, not before it.
  - Cannot be used when piping input from stdin.
  - Example: `renegade -i "explain this code"`
...
- **`--approval-mode <mode>`**:
  ...
  - Example: `renegade --approval-mode auto_edit`
- **`--allowed-tools <tool1,tool2,...>`**:
  - A comma-separated list of tool names that will bypass the confirmation
    dialog.
  - Example: `renegade --allowed-tools "ShellTool(git status)"`
- **`--extensions <extension_name ...>`** (**`-e <extension_name ...>`**):
  - Specifies a list of extensions to use for the session. If not provided, all
    available extensions are used.
  - Use the special term `renegade -e none` to disable all extensions.
  - Example: `renegade -e my-extension -e my-other-extension`
- **`--list-extensions`** (**`-l`**):
...
- **`--resume [session_id]`** (**`-r [session_id]`**):
  ...
  - Example: `renegade --resume 5` or `renegade --resume latest` or
    `renegade --resume a1b2c3d4-e5f6-7890-abcd-ef1234567890` or `renegade --resume`
...
- **`--list-sessions`**:
  ...
  - Shows session indices, dates, message counts, and preview of first user
    message.
  - Example: `renegade --list-sessions`
- **`--delete-session <identifier>`**:
  ...
  - Use `--list-sessions` first to see available sessions, their indices, and
    UUIDs.
  - Example: `renegade --delete-session 3` or
    `renegade --delete-session a1b2c3d4-e5f6-7890-abcd-ef1234567890`
...
(defaulting to `renegade.md` but configurable via the `context.fileName` setting)
are crucial for configuring the _instructional context_ (also referred to as
"memory") provided to the Renegade Kernel model. This powerful feature allows you to give
...
- **Purpose:** These Markdown files contain instructions, guidelines, or context
  that you want the Renegade Kernel model to be aware of during your interactions. The
...
- **Hierarchical loading and precedence:** The CLI implements a sophisticated
  ...
  1.  **Global context file:**
      - Location: `~/.renegade/<configured-context-filename>` (e.g.,
        `~/.renegade/renegade.md` in your user home directory).
...
- **Concatenation and UI indication:** The contents of all found context files
  ...
  provided as part of the system prompt to the Renegade Kernel model. The CLI footer
...
the Renegade Kernel's responses to your specific needs and projects.

## Sandboxing

The Renegade Kernel can execute potentially unsafe operations (like shell commands
...
By default, it uses a pre-built `renegade-cli-sandbox` Docker image.

For project-specific sandboxing needs, you can create a custom Dockerfile at
`.renegade/sandbox.Dockerfile` in your project's root directory. This Dockerfile
can be based on the base sandbox image:

```dockerfile
FROM renegade-cli-sandbox
...
When `.renegade/sandbox.Dockerfile` exists, you can use `BUILD_SANDBOX`
environment variable when running Renegade Kernel to automatically build the custom
sandbox image:

```bash
BUILD_SANDBOX=1 renegade -s
```

## Usage statistics

To help us improve the Renegade Kernel, we collect anonymized usage statistics. This
...
- **API requests:** We log the Renegade Kernel model used for each request, the duration
...
- **Prompt and response content:** We do not log the content of your prompts or
  the responses from the Renegade Kernel model.
