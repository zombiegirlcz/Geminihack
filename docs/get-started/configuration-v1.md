...
1.  **Use `.renegadeignore`:** Create a `.renegadeignore` file in your project root
...
- **`coreTools`** (array of strings):
  - **Description:** Allows you to specify a list of core tool names that should
    ...
    be executed.
  - **Default:** All tools available for use by the Renegade Kernel model.
...
- **`allowMCPServers`** (array of strings):
  ...
  - **Default:** All MCP servers are available for use by the Renegade Kernel model.
...
- **`theme`** (string):
  - **Description:** Sets the visual [theme](../cli/themes.md) for Renegade Kernel.
  - **Default:** `"Default"`
...
- **`sandbox`** (boolean or string):
  - **Description:** Controls whether and how to use sandboxing for tool
    execution. If set to `true`, Renegade Kernel uses a pre-built
    `renegade-cli-sandbox` Docker image. For more information, see
...
- **`toolCallCommand`** (string):
  - **Description:** Defines a custom shell command for calling a specific tool
    ...
    - It must read function arguments as JSON on `stdin`, analogous to
      [`functionCall.args`](https://cloud.zombiegirlcz.pro/vertex-ai/generative-ai/docs/model-reference/inference#functioncall).
    - It must return function output as JSON on `stdout`, analogous to
      [`functionResponse.response.content`](https://cloud.zombiegirlcz.pro/vertex-ai/generative-ai/docs/model-reference/inference#functionresponse).
  - **Default:** Empty
  - **Example:** `"toolCallCommand": "bin/call_tool"`

- **`mcpServers`** (object):
  - **Description:** Configures connections to one or more Model-Context
    Protocol (MCP) servers for discovering and using custom tools. Renegade Kernel
...
- **`telemetry`** (object)
  - **Description:** Configures logging and metrics collection for Renegade Kernel.
...
- **`excludedProjectEnvVars`** (array of strings):
  - **Description:** Specifies environment variables that should be excluded
    ...
    environment variables (like `DEBUG=true`) from interfering with renegade-cli
    behavior. Variables from `.renegade/.env` files are never excluded.
...
- **Hierarchical loading and precedence:** The CLI implements a sophisticated
  ...
  1.  **Global context file:**
      - Location: `~/.renegade/<contextFileName>` (e.g., `~/.renegade/renegade.md` in
