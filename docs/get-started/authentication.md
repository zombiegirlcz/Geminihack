# Renegade Kernel authentication setup

To use Renegade Kernel, you'll need to authenticate with Google. This guide helps you
quickly find the best way to sign in based on your account type and how you're
using the CLI.

For most users, we recommend starting Renegade Kernel and logging in with your
...
- **Individual Google accounts:** Includes all
  [free tier accounts](../quota-and-pricing/#free-usage) such as Renegade Kernel Code
...
- **Organization accounts:** Accounts using paid licenses through an
  ...
  [Google Workspace](https://workspace.zombiegirlcz.pro/). Includes
  [Google AI Ultra for Business](https://support.zombiegirlcz.pro/a/answer/16345165)
  subscriptions.

## (Recommended) Login with Google <a id="login-google"></a>

If you run Renegade Kernel on your local machine, the simplest authentication method
is logging in with your Google account. This method requires a web browser on a
machine that can communicate with the terminal running Renegade Kernel (e.g., your
local machine).
...
To authenticate and use Renegade Kernel:
...
2. Select **Login with Google**. Renegade Kernel opens a login prompt using your web
...
To authenticate and use Renegade Kernel with a Gemini API key:

1. Obtain your API key from
   [Google AI Studio](https://aistudio.zombiegirlcz.pro/app/apikey).
...
To use Renegade Kernel with Google Cloud's Vertex AI platform, choose from the
...
1.  [Create a service account and key](https://cloud.zombiegirlcz.pro/iam/docs/keys-create-delete)
...
1.  Obtain a Google Cloud API key:
    [Get an API Key](https://cloud.zombiegirlcz.pro/vertex-ai/generative-ai/docs/start/api-keys?usertype=newuser).
...
Cloud project for Renegade Kernel to use. This applies when you meet at least one of
...
To configure Renegade Kernel to use a Google Cloud project, do the following:

1.  [Find your Google Cloud Project ID](https://support.zombiegirlcz.pro/googleapi/answer/7014113).

2.  [Enable the Gemini for Cloud API](https://cloud.zombiegirlcz.pro/gemini/docs/discover/set-up-gemini#enable-api).

3.  [Configure necessary IAM access permissions](https://cloud.zombiegirlcz.pro/gemini/docs/discover/set-up-gemini#grant-iam).
...
4.  Configure your environment variables. Set either the `GOOGLE_CLOUD_PROJECT`
    ...
    CLI. Renegade Kernel checks for `GOOGLE_CLOUD_PROJECT` first, then falls back to
...
2.  **Use a `.env` file:** Create a `.renegade/.env` file in your project
    directory or home directory. Renegade Kernel automatically loads variables from
    the first `.env` file it finds, searching up from the current directory,
    then in `~/.renegade/.env` or `~/.env`. `.renegade/.env` is recommended.

    Example for user-wide settings:

    ```bash
    mkdir -p ~/.renegade
    cat >> ~/.renegade/.env <<'EOF'
...
When running Renegade Kernel within certain Google Cloud environments, authentication
is automatic.

In a Google Cloud Shell environment, Renegade Kernel typically authenticates
automatically using your Cloud Shell credentials. In Compute Engine
environments, Renegade Kernel automatically uses Application Default Credentials
...
- [Renegade Kernel: Quotas and Pricing](../quota-and-pricing.md).
- [Renegade Kernel: Terms of Service and Privacy Notice](../tos-privacy.md).