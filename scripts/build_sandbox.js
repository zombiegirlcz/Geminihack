...
if (!image.length) {
  console.warn(
    'No default image tag specified in renegade-cli/packages/cli/package.json',
  );
}
...
console.log('packing @zombiegirlcz/RENEGADEkarnelcli ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'google-renegade-cli-*.tgz'), { force: true });
execSync(
  `npm pack -w @zombiegirlcz/RENEGADEkarnelcli --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing @zombiegirlcz/RENEGADEkarnelcli-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'google-renegade-cli-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @zombiegirlcz/RENEGADEkarnelcli-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `google-renegade-cli-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `google-renegade-cli-core-${packageVersion}.tgz`),
  0o755,
);
...
function buildImage(imageName, dockerfile) {
  ...
  if (sandboxCommand === 'podman') {
    if (isWindows) {
      ...
      tempAuthFile = join(os.tmpdir(), `renegade-auth-${Date.now()}.json`);
